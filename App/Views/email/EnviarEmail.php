<?php	
	...
	$mensagem .=  "<tr><td>";
		    foreach($this->views->cor as $tag):
		            if ($tarefa['id'] == $tag['id']){  
		           $mensagem .=  "<span title='". $tag['nome']."' style=' margin-right:5px; height:25px; width: 25px; display:inline-block; background:".$tag['cor']."'></span>";
		         } 
		         endforeach ;
	      $mensagem .=	"</td><td>". $tarefa['nome'] ."</td>";
   ...
   $mensagem .="<td>".  $prioridade ."</td>
	        	<td>".  $tarefa['tempoEstimado'] ." minutos </td>
	...
	$mail->FromName = "teste123456789"; // Seu nome
